...
void loadfiles(const char* filename1, const char* filename2) {
    ...
    if (ignorecase){
        for (int i = 0, j = 0; i < count1 && j < count2;  ++i, ++j) {
            for(int x = 0, y = 0; x < strlen(strings1[i]) && y <strlen(strings2[j]); ++x, ++y){
                strings1[i][x] = tolower(strings1[i][x]);
                strings2[j][y] = tolower(strings2[j][y]);
            }
        }
    }
...
void version(void) {
    ...
    printf("Written by William McCarthy, Tony Stark, Dr. Steven Strange, and Vinh Le\n\n");
}
            ...
            if(foundmatch == 2){
                ...
                while(i < p->stop && j < q->stop){
                    if(strcmp(strings1[i],strings2[j]) != 0){
                        print_different(strings1[i], strings2[j]);
                    }
                    else if(showleftcolumn){
                        print_left_column(strings1[i]);
                    }
                    else if(suppresscommon){}
                    else{
                        printboth(strings1[i]);
                    }
                    i++;
                    j++;
                }
                ...
                if(showleftcolumn){
                para_print(p, print_left_column);
                }
                else if(suppresscommon){}
                else{
...
void show_context(const char* file1, const char* file2){
    ...
    while (p != NULL) {
        ...
        if (foundmatch) {
            while ((foundmatch = para_equal(p, q)) == 0) {
                print_line_num(q->start, q->stop, "right");
                para_print(q, print_add);
                q = para_next(q);
                qlast = q;
            }
            if(foundmatch == 2){
                int i = p->start;
                int j = q->start;
                print_line_num(p->start, p->stop, "left");
                while(i < p->stop && j < q->stop){
                    if(strcmp(strings1[i],strings2[j]) != 0){
                        print_subtract(strings1[i]);
                    }
                    else{
                        print_line(strings1[i]);
                    }
                    i++;
                    j++;
                }
        ...
        q = para_next(q);
            ...
            print_line_num(p->start,p->stop,"left");
            para_print(p, print_subtract);
            p = para_next(p);
        }
    }
    while (q != NULL) {
        print_line_num(q->start, q->stop, "right");
        para_print(q, printadd);
        q = para_next(q);
    }*/
}
...
void init_options_files(int argc, const char* argv[]) {
  ...
  const char* files[2] = { NULL, NULL };
    ...
    if (showcontext){
        show_context(files[0], files[1]);
        exit(0);
    }
    if (showunified){
        show_unified(files[0], files[1]);
        exit(0);
    }
    if (showsidebyside){ 
        sidebyside(); 
